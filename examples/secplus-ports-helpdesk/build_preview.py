#!/usr/bin/env python3
"""Build self-contained portland-desk.html from narrative + glossary."""
...
UNIT = ROOT / "unit.yaml"
OUT = ROOT / "portland-desk.html"
OUT_COPY = Path("/workspace/portland-desk-preview.html")
...
def md_to_html(md: str, terms: dict) -> tuple[str, list[str]]:
    lines = md.splitlines()
    used_ids: list[str] = []
    out: list[str] = []
    i = 0
    # Skip leading H1 (page title comes from unit) and byline italic if present
    # We'll render title from unit; include byline from first italic line after h1
    title = None
    byline = None
    body_start = 0
    if lines and lines[0].startswith("# "):
        ...
        while body_start < len(lines) and not lines[body_start].strip():
            body_start += 1
        if body_start < len(lines) and lines[body_start].startswith("*") and lines[body_start].endswith("*"):
...
def build():
    unit = yaml.safe_load(UNIT.read_text(encoding="utf-8"))
    bank, terms = load_glossary()
    md = NARRATIVE.read_text(encoding="utf-8")
    title, byline, body_html, used_ids = md_to_html(md, terms)

    page_title = unit.get("title") or title or "Portland Desk"
    byline_note = (
    ...
    if byline:
        ...
        subtitle = "A Security+ ports-and-protocols precursor"

    # Validate every used id exists (already raised); also unused warning
    ...
    # Glossary JSON for embed (full fields for sidebar; tooltip uses subset in JS)
    gloss_list = [terms[tid] for tid in sorted(terms.keys(), key=lambda k: terms[k]["title"].lower())]
...
"""
    ...
    OUT_COPY.write_text(html_out, encoding="utf-8")
    ...
    return exam_marks, support_marks, used_ids, terms
